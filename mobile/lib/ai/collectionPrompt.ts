...
// Ported from src/lib/ai/collectionPrompt.ts - shared by groq.ts and
// gemini.ts's generateSmartCollections. A fixed prompt against the same
// library tends to converge on the same "obvious" groupings every time
// (temperature alone isn't enough: the model keeps reaching for the same
// handful of natural genre clusters). Three things push it toward
// genuinely different results on repeat generations:
