...
export class GoogleBooksClient {
  ...
  async init() {
    ...
    this.apiKey = (await resolveApiKey('google_books_key', process.env.EXPO_PUBLIC_GOOGLE_BOOKS_API_KEY)) || null;
  ...
  private async fetch<T>(endpoint: string, signal?: AbortSignal): Promise<T | null> {
    ...
    try {
      ...
      const response = await fetch(`${GOOGLE_BOOKS_BASE_URL}${endpoint}${keyParam}`, { signal });
      if (!response.ok) {
  ...
  private normalizeBook(item: GoogleBooksResult): SearchResult {
    ...
    const year = info.publishedDate ? parseInt(info.publishedDate.split('-')[0]) : null;
    ...
    return {
      ...
      release_year: year,
      api_rating: info.averageRating ? info.averageRating * 2 : null,
  ...
  private determineBookType(categories: string[] | undefined): 'book' | 'light_novel' | 'visual_novel' {
    ...
    if (cats.includes('light novel') || cats.includes('lightnovel')) return 'light_novel';
    if (cats.includes('visual novel') || cats.includes('visualnovel')) return 'visual_novel';
    return 'book';
  }
}

export const createBooksClient = () => new GoogleBooksClient();
