...
import type { Friendship, FriendshipWithProfile, FriendActivityEntry } from '@/types';

// Friends are inherently an online-only feature (per the Phase 2 plan) -
// no Dexie caching, no offline queue. Every action goes straight to
// Supabase; a failure just surfaces as an error rather than getting queued.
...
// 8 unambiguous, case-insensitive-safe characters (no 0/O, 1/I/L, no
// vowSpelling-word risk) - short enough to type/read off a screen, long
// enough (32^8 ≈ 1 trillion combinations) that guessing one isn't
// practical, matching the "the code is the credential" security model
// used throughout (see the schema.sql comment on friend_invite_codes).
...
export const useFriendStore = create<FriendStore>()((set, get) => ({
  ...
  fetchFriendsActivity: async () => {
    ...
    try {
      ...
      // it directly - one round trip for both the activity and its title.
      // RLS applies to the embedded table too, so this only ever returns
      // media rows the "select friends media" policy already permits.
      const { data, error } = await (supabase as any)
        .from('history')
  ...
  sendRequest: async (email) => {
    ...
    set({ isLoading: true, error: null });
    try {
      const findRes = await fetch('/api/friends/find', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!findRes.ok) {
        return { success: false, message: found.error || 'User not found.' };
      }
      ...
      // that exact order, so without this check a reverse-direction request
      // could silently create a duplicate/conflicting relationship.
  ...
  fetchMyInviteCode: async () => {
    ...
    try {
      ...
      // UNIQUE(code) constraint is astronomically unlikely at 8 chars from
      // a 32-symbol alphabet, but retried a few times regardless rather
      // than surfacing a raw constraint error for what's really just bad
      // luck on one random draw.
  ...
  previewInviteCode: async (code) => {
    try {
      const res = await fetch('/api/friends/preview-code', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const json = await res.json();
      if (!res.ok) {
        return { success: false, message: json.error || 'Invalid invite code.' };
      }
      return { success: true, profile: json as InvitePreview };
    } catch (e) {
